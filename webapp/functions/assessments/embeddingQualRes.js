...
const firestoreService = {
	async getJobDocument(googleId, docId) {
		try {
			const jobDoc = await db.collection('users').doc(googleId).collection('jobs').doc(docId).get();
	...
	async batchUpdateQualityScores(googleId, docId, updates) {
		try {
			...
			const docRef = db.collection('users').doc(googleId).collection('jobs').doc(docId);
...
const matchingService = {
	async processMatch(googleId, docId) {
		try {
			...
			const filteredScores = pairedScores.filter(
				(item) => item.score >= CONFIG.matching.similarityThreshold
			);

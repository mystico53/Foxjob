...
const RETRY_DELAYS = [1 * MINUTE_IN_MS, 2 * MINUTE_IN_MS, 3 * MINUTE_IN_MS];
...
async function retryWithBackoff(operation, attemptNumber = 0) {
	try {
		...
		const isRateLimit =
			error.status === 429 || error.message?.includes('rate') || error.message?.includes('quota');
		...
		if (isRateLimit || isServerError) {
			...
			logger.info(
				`Rate limited or server error. Retrying in ${delayMinutes} minute(s). Retries left: ${MAX_RETRIES - attemptNumber}`
			);

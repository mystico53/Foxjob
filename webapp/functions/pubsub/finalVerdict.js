...
const services = {
		...
		extractFields(obj, prefix = '') {
			...
			for (const [key, value] of Object.entries(obj)) {
				...
				if (
					...
					fieldPath.match(
						/SkillAssessment\.DomainExpertise\.(assessment|importance|name|score|summary)/
					) // Skip specific DomainExpertise fields
				) {
					continue;
				}
	...
	api: {
        ...
        Fields:
        ${documentData.fields.map((field) => `${field.path}: ${field.value}`).join('\n')}
      `;
...
exports.finalVerdict = onMessagePublished(
	...
	async (event) => {
		try {
			...
			logger.info(
				`Processing status updated to "processed" in generalData for job ID: ${docId}, user ID: ${firebaseUid}`
			);

...
const skillsProcessor = {
	...
	formatSkillAssessment(matchResult, originalHardSkills) {
		try {
			const parsedResult = typeof matchResult === 'string' ? JSON.parse(matchResult) : matchResult;
...
exports.matchHardSkills = onMessagePublished(
	...
	async (event) => {
		try {
			...
			const { ref: jobDocRef, data: jobData } = await firestoreService.getJobDocument(
				firebaseUid,
				docId
			);

			// Process hard skills
			const hardSkillsArray = skillsProcessor.extractHardSkillsArray(
			...
			await pubSubService.publishMessage(CONFIG.topics.hardSkillsMatched, { firebaseUid, docId });

...
exports.extractDomainExpertise = onMessagePublished(
	...
	async (event) => {
		...
		logger.info(
			`Starting domain expertise extraction for firebaseUid: ${firebaseUid}, docId: ${docId}`
		);
		...
		try {
			...
			logger.info(
				`Domain expertise saved to Firestore for firebaseUid: ${firebaseUid}, docId: ${docId}`
			);

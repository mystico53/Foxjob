...
exports.onBatchUpdate = onDocumentUpdated('jobBatches/{batchId}', async (event) => {
	try {
		...
		if (
			afterData.completedJobs >= afterData.totalJobs &&
		...
		// Handle case where batch is already marked as complete or timeout, but doesn't have emailSent
			...
			(afterData.status === 'complete' || afterData.status === 'timeout') &&
			!beforeData.emailSent &&
			afterData.emailSent
		) {
			logger.info('Batch marked as complete/timeout with emailSent=true, sending email', {
				batchId
			});
...
exports.fixMissingEmailRequests = onCall(
	...
	async (request) => {
		try {
			...
			const batchesQuery = await db.collection('jobBatches').where('emailSent', '==', true).get();
			...
			for (const batch of missingBatches) {
				try {
					...
					const emailRequestData = {
						...
						createdAt:
							batch.completedAt || batch.startedAt || admin.firestore.FieldValue.serverTimestamp(),
						sentAt:
							batch.completedAt || batch.startedAt || admin.firestore.FieldValue.serverTimestamp(),
						opened: false,
						openCount: 0,
					...
					logger.info(
						`Successfully created email request ${emailRequestRef.id} for batch ${batchId}`
					);
					fixedCount++;
				} catch (err) {

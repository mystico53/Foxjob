  ...
  console.log("ENTRY_LOG - searchBright function entry point", {
    ...
    isScheduled: req.body && req.body.schedule ? "YES" : "NO"
		...
		return cors(req, res, async () => {
			...
			try {
				const { userId, searchParams, limit, schedule, resume } = req.body;
				...
				if (schedule) {
					...
					try {
						...
						const minimumScore =
							req.body.minimumScore !== undefined ? parseInt(req.body.minimumScore, 10) : 70;
      ...
      console.log("Quoted keyword:", requestData.keyword);*/
				...
				if (schedule) {
					try {
						...
						// Update search result and set searching status
						await docRef.update({
							lastRunResult: response.data,
							searchingStatus: 'searching',
							lastSearchStarted: FieldValue.serverTimestamp()
						});
					} catch (updateError) {

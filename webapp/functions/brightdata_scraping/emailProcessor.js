...
exports.processEmailRequests = onDocumentCreated(
	...
	async (event) => {
				...
				if (isEmptySearchNotification) {
					...
					try {
						...
						if (snapshot.empty) {
							...
							if (emailData.searchId) {
								try {
									...
									if (searchQueryDoc.exists) {
										...
										logger.info(
											`Retrieved minimum score threshold for search query: ${minimumScore}`,
											{
												userId: uid,
												searchId: emailData.searchId
											}
								...
								try {
									...
									if (batchDoc.exists && batchDoc.data().searchId) {
										...
										if (searchQueryDoc.exists) {
											...
											minimumScore = searchQueryDoc.data().minimumScore || 0;
											logger.info(
												`Retrieved minimum score threshold from batch's search query: ${minimumScore}`,
												{
													userId: uid,
													batchId: emailData.batchId,
													searchId: searchId
									...
									logger.error(
										'Error retrieving batch document or associated search query:',
										error
									);
									// Continue with default minimumScore
								}
							...
							logger.info(
								`Query returned ${jobsSnapshot.size} documents after filtering with minimum score ${minimumScore}`
							);
                  ...
                  `;
									...
									topJobs.forEach((job, index) => {
										const doc = job.doc;
										logger.info(
											`Processing job ${index + 1} with ID ${doc.id}, score: ${doc.data().match?.final_score || 'N/A'}`
										);
										...
										const description =
											jobData.match?.summary?.short_description ||
											'No company description available';
										const responsibility =
											jobData.match?.summary?.short_responsibility ||
											'No responsibility description available';
										const gaps =
											jobData.match?.summary?.short_gaps || 'No gaps information available';
										const preferenceScore = jobData.match?.preferenceScore?.score || 'N/A';
										const preferenceExplanation =
											jobData.match?.preferenceScore?.explanation || 'No preference data available';
      ...
      `;
			...
			const finalText = isEmptySearchNotification
				? jobsText
				: `Hello ${userFirstName},
...
${jobsText}`;
			...
			const msg = {
				...
				subject: isEmptySearchNotification
					? emailData.subject || 'Email from Foxjob'
					: dynamicSubject,
				text: finalText,
				html: finalHtml,
				trackingSettings: {

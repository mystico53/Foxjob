...
function createEmailContent(originalHtml, originalText, jobsData = []) {
	...
	if (jobsData.length > 0) {
		...
		jobsData.forEach((job) => {
			...
			const description =
				job.match?.summary?.short_description ||
				(typeof job.match?.summary === 'string'
					? job.match.summary
					: 'No company description available');
			const responsibility =
				job.match?.summary?.short_responsibility || 'No responsibility description available';
		...
		function getMockJobData() {
			return {
				...
				match: {
					...
					summary: {
						short_description:
							'TechCorp is a leading software company specializing in AI solutions.',
						short_responsibility:
							'Building and maintaining web applications using modern JavaScript frameworks.',

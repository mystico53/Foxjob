	...
	for (let attempt = 1; attempt <= maxRetries + 1; attempt++) {
		try {
			...
			if (attempt <= maxRetries) {
				...
				logger.warn(
					`${operationName} failed (attempt ${attempt}/${maxRetries + 1}), retrying in ${Math.round(backoffDelay)}ms`,
					{
		...
		try {
			...
			if (req.body?.snapshot_id && req.body?.status === 'ready') {
				...
				try {
					...
					const brightdataToken = secretVersion.payload.data.toString().trim();
					...
					if (!jobs || jobs.length === 0) {
						...
						// Update searchingStatus to 'completed' for empty search (first case)
						if (searchId) {
							try {
								const searchRef = db
									.collection('users')
								...
								logger.error('Failed to update search query status for empty search (first case)', {
									error: error.message,
									searchId,
									userId
								});
					...
					// Update searchingStatus to 'completed' for the associated search query
					if (searchId) {
						try {
							const searchRef = db
								.collection('users')
							...
							logger.error('Failed to update search query status', {
								error: error.message,
								searchId,
								userId
							});
					...
					if (
						error.message &&
						(error.message.includes('Failed to download snapshot') ||
						...
						// Update searchingStatus to 'completed' for empty search (error case)
						if (searchId) {
							try {
								const searchRef = db
									.collection('users')
								...
								logger.error('Failed to update search query status for empty search (error case)', {
									error: error.message,
									searchId,
									userId
								});

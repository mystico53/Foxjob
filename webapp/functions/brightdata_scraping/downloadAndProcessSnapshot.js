...
async function downloadSnapshot(snapshotId, authToken) {
	try {
		...
		logger.info(
			`BrightData API response received in ${requestDuration}ms for snapshot ${snapshotId}`,
			{
		...
		if (Array.isArray(response.data) && response.data.length > 0) {
			...
			if (sampleJob.job_description_formatted)
				sampleJob.job_description_formatted = '[REDACTED FOR LOG]';
			...
			const titleAnalysis = jobTitles.reduce(
				(acc, title) => {
		...
		try {
			...
			if (!brightdataToken) {
				...
				try {
					...
					const [brightdataVersion] = await secretManager.accessSecretVersion({
						name: brightdataTokenName
					});
			...
			for (let i = 0; i < jobs.length; i += MAX_BATCH_SIZE) {
				...
				logger.info(
					`Processing batch ${Math.floor(i / MAX_BATCH_SIZE) + 1} with ${batchJobs.length} jobs`
				);

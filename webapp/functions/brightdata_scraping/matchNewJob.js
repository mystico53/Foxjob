...
const embeddingService = {
	...
	async getEmbeddings(texts) {
		...
		const preprocessedTexts = Array.isArray(texts)
			? texts.map(this.preprocessText)
			: [this.preprocessText(texts)];
...
const matchingService = {
	async processMatch(jobDescription, resumeText) {
		try {
			...
			const similarity = embeddingService.calculateSimilarity(jobEmbedding[0], resumeEmbedding[0]);
...
exports.matchNewJob = onMessagePublished(
	...
	async (event) => {
		try {
			...
			const matchScore = await matchingService.processMatch(jobDescription, resumeText);

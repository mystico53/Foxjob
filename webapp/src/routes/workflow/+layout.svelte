<script>
    ...
    async function toggleBookmark(jobId) {
        ...
        try {
            ...
            const newStatus = jobToUpdate.generalData?.status?.toLowerCase() === 'bookmarked' ? 'read' : 'bookmarked';
...
<div class="fixed left-0 flex h-[calc(100vh-72px)] w-full flex-col md:flex-row">
    <aside class="bg-surface-100 custom-scrollbar h-full w-full transform overflow-y-auto border-r transition-transform md:w-80 md:translate-x-0"
        style="width: 25rem;"
        bind:this={sidebar}
    >
        <SortControls />

	...
	async function fetchJobData() {
		...
		try {
			...
			const jobPromises = jobDocs.map(async (jobDoc, index) => {
				console.log(
					`fetchJobData: Processing job ${index + 1}/${jobDocs.length} with ID: ${jobDoc.id}`
				);
				try {
			...
			console.log(
				`fetchJobData: Sorting job data by column: ${sortColumn}, direction: ${sortDirection}`
			);
				...
				{#each jobData as job}
					<div class="card">
						...
						<div class="card-footer">
							...
							<button on:click={() => openJobLink(job.generalData?.url)} class="view-button"
								>View Job</button
							>
						</div>
					</div>

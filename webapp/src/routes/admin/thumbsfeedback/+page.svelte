...
<div class="fixed left-0 flex h-[calc(100vh-72px)] w-full flex-col">
    ...
    <div class="bg-surface-100 border-b p-4 flex items-center gap-4">
        <button class="btn btn-sm variant-ghost" on:click={goBack}>
...
<div class="fixed left-0 flex h-[calc(100vh-72px)] w-full flex-col md:flex-row">
    ...
    >
        <div class="p-4">
            ...
            {:else if !loading}
                <div class="text-surface-400 p-4 text-center">No feedback found.</div>
            {/if}
        </div>
    </aside>

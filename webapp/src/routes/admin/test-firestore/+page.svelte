<script>
	...
	async function testFirestoreConfig() {
		const config = {
			...
			emulator:
				db._settings?.host?.includes('localhost') || db._settings?.host?.includes('127.0.0.1'),
			authDomain: db.app.options.authDomain
		};
...
<div class="container mx-auto p-4">
	...
	<p class="mb-4 text-amber-500">⚠️ This page is for debugging Firestore access issues.</p>

	<div class="card mb-4 p-4">
		<div class="mb-2 flex items-center justify-between">
			<h2 class="h3">Admin Claims</h2>
			<span
				class="badge {environment === 'local-emulator'
					? 'bg-primary'
					: environment === 'staging'
						? 'bg-warning'
						: environment === 'production'
							? 'bg-success'
							: 'bg-surface'}">{environment}</span
			>
		</div>
		<pre class="bg-surface-200-700-token mb-4 rounded p-2 text-sm">{JSON.stringify(
				adminClaims,
				null,
				2
			)}</pre>
		...
		<div class="flex gap-2">
			<button class="variant-filled-primary btn" on:click={runTests} disabled={isLoading}>
				{isLoading ? 'Running Tests...' : 'Run All Tests'}
			</button>
			<button class="variant-filled-secondary btn" on:click={refreshToken} disabled={isLoading}>
				Refresh Token
			</button>
		</div>
	</div>
	...
	{:else if testResults.length > 0}
		<div class="grid gap-4">
			{#each testResults as result}
				<div
					class="card p-4 {result.status === 'error'
						? 'variant-soft-error'
				...
				>
					...
					{#if result.data}
						<details class="mt-2">
							...
							<pre
								class="bg-surface-200-700-token mt-2 max-h-40 overflow-auto rounded p-2 text-xs">{result.data}</pre>
						</details>
					{/if}
				</div>
			{/each}
...
<style>
	.badge {
		@apply rounded px-2 py-1 text-sm text-white;
	}

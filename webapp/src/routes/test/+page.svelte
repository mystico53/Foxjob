...
<div class="container mx-auto p-4">
	...
	<div class="mb-8 flex flex-wrap gap-4">
		<button class="variant-filled-primary btn" on:click={testFirestore}> Test Firestore </button>

		{#if !$authStore}
			<button class="variant-filled-secondary btn" on:click={testAuth}> Test Sign In </button>
		{:else}
			<button class="variant-filled-warning btn" on:click={testSignOut}> Test Sign Out </button>
		{/if}

		<button class="variant-filled-tertiary btn" on:click={testCloudFunction}>
			Test Cloud Function
		</button>

		<button class="variant-ghost-surface btn" on:click={clearResults}> Clear Results </button>
	</div>

	<div class="space-y-4">
		{#each testResults as result}
			<div
				class="card p-4 {result.status === 'success'

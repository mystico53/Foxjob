<footer class="py-24 text-center">
	<p class="text-sm text-gray-500">
		© 2024 FoxJob. All rights reserved.
		<a class="privacy" href="/privacy" data-sveltekit-preload-data="off"> Privacy Policy </a>
		{' / '}
		<a class="terms" href="/terms" data-sveltekit-preload-data="off"> Terms of Service </a>
	</p>
	<p class="text-sm text-gray-500">
		Version: 1.53 (
		<a
			class="patchnotes"
			href="https://spiritual-chicken-5d2.notion.site/Foxjob-Patch-Notes-1e3370f58e75805ebbd4e914235b4436"
			target="_blank"
			rel="noopener noreferrer"
		>
			Patchnotes
		</a>
		)
	</p>
</footer>

<style>
	/* You can add any custom styles for the footer links here */
	footer a {
		color: #6b7280;
		text-decoration: underline;
		transition: color 0.2s;
	}

	footer a:hover {
		color: #4b5563;
	}
</style>

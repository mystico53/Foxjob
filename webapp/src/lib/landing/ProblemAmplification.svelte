	...
	<div class="flex w-full flex-col items-center justify-center md:flex-row">
		...
		<div class="flex items-center p-12">
			<svg
				bind:this={donutRef}
				viewBox="0 0 120 120"
				class="z-20 h-40 w-40 drop-shadow-xl"
				style="filter:drop-shadow(0 4px 16px rgba(241,90,34,0.12)); opacity: {donutVisible
					? 1
			...
			>
				...
				<text
					...
					y="54%"
					text-anchor="middle"
					class="fill-[#F15A22] text-4xl font-normal"
		...
		<!-- Right: Text -->
		<div
			class="ml-2 flex flex-col items-center justify-center text-center md:items-start md:text-left"
		>
			<p class="py-4 text-2xl font-bold text-gray-700 dark:text-gray-300">
				<span class="uncover" class:uncover-active={donutVisible} style="display: inline-block;"
					>Missed</span
				> opportunities
			</p>
			<p class="font-heavy text-lg text-gray-700 dark:text-gray-300">
				83% of professionals find that reading job descriptions takes too much time
			</p>
		</div>
	</div>
</div>
...
<style>
	...
	.uncover {
		...
		opacity: 0;
		transition:
			clip-path 2s cubic-bezier(0.4, 2, 0.6, 1),
			opacity 0.4s;
	}

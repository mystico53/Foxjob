						...
						(snapshot) => {
							const queries = snapshot.docs.map((doc) => {
								...
								return {
									...
									lastSearchStarted: data.lastSearchStarted?.toDate() || null,
									lastSearchCompleted: data.lastSearchCompleted?.toDate() || null
								};
							});
	...
	const refreshQuery = async (userId, queryId) => {
		...
		try {
			...
			if (docSnap.exists()) {
				...
				update((state) => {
					const newQueries = state.queries.map((q) => (q.id === queryId ? updatedQuery : q));
...
export const activeQueriesStore = derived(searchQueriesStore, ($searchQueriesStore) =>
	$searchQueriesStore.queries.filter((query) => query.isActive)
);

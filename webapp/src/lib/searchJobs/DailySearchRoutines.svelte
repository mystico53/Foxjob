<script>
	...
	async function setupRealtimeListener() {
		...
		try {
			...
			const { getFirestore, collection, query, where, orderBy, onSnapshot, limit } = await import(
				'firebase/firestore'
			);
	...
	async function saveChanges() {
		...
		try {
			...
			// Update the document
			await updateDoc(doc(db, 'users', uid, 'searchQueries', searchId), {
				frequency: editFrequency,
...
<div class="container mx-auto mt-8 p-4">
	<h2 class="mb-4 text-2xl font-bold">Your Active Job Agents</h2>
	...
	{:else if scheduledSearches.length === 0}
		<div class="alert variant-filled-surface mb-4" transition:fade={{ duration: 200 }}>
			<span
				>You don't have any scheduled searches yet. Toggle "Email me new matching jobs daily" when
				searching to create one.</span
			>
		</div>
	{:else}
		...
		<div class="grid gap-4">
			{#each scheduledSearches as search (search.id)}
				<div class="card variant-filled-surface p-4" transition:slide={{ duration: 300 }}>
						...
						<form on:submit|preventDefault={saveChanges} class="space-y-4">
							...
							<div class="grid grid-cols-1 gap-4 md:grid-cols-2">
								<div class="form-field">
									...
									<select id="editFrequency" bind:value={editFrequency} class="select w-full">
							...
							<div class="flex justify-end space-x-2">
								...
								<button
									...
									disabled={isUpdating || operationsInProgress.has(`save-${search.id}`)}
								>
									{isUpdating || operationsInProgress.has(`save-${search.id}`)
										? 'Saving...'
										: 'Save Changes'}
								</button>
							</div>
							...
							<div class="flex items-start justify-between">
								...
								<div class="flex space-x-2">
									<button
										...
										disabled={operationsInProgress.has(`edit-${search.id}`) ||
											operationsInProgress.has(`delete-${search.id}`)}
									>
										Edit
									</button>
									<button
							...
							<div class="grid grid-cols-1 gap-2 text-sm md:grid-cols-2">
								...
								<div class="space-y-1">
									<div>
										...
										{getFrequencyText(search.frequency)}
									</div>
									<div>
									...
									<div>
										...
										{formatDate(search.lastRun)}
									</div>
									<div>
										<span class="font-semibold">Next run:</span>
										{formatDate(search.nextRun)}
									</div>
								</div>
								...
								<div class="space-y-1">
									{#if search.searchParams[0].job_type}
										<div>
											...
											{search.searchParams[0].job_type}
										</div>
									{/if}
									...
									{#if search.searchParams[0].experience_level}
										<div>
											...
											{search.searchParams[0].experience_level}
										</div>
									{/if}

									{#if search.searchParams[0].remote}
										<div>
											<span class="font-semibold">Workplace:</span>
											{search.searchParams[0].remote}
										</div>
									{/if}

									{#if search.searchParams[0].time_range && search.searchParams[0].time_range !== 'Any time'}
										<div>
											<span class="font-semibold">Date Posted:</span>
											{search.searchParams[0].time_range}
										</div>
									{/if}

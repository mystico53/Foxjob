<script>
	...
	async function searchJobs() {
		...
		try {
			...
			const response = await fetch('http://127.0.0.1:5001/jobille-45494/us-central1/searchBright', {
				method: 'POST',
				headers: {
...
<div class="container mx-auto p-4">
	<form on:submit|preventDefault={searchJobs} class="space-y-4">
		...
		<div>
			...
			<select id="jobType" bind:value={jobType} class="w-full rounded border p-2">
		...
		<div>
			...
			<select id="experience" bind:value={experience} class="w-full rounded border p-2">
		...
		<div>
			...
			<select id="workplaceType" bind:value={workplaceType} class="w-full rounded border p-2">
		...
		<div>
			...
			<select id="datePosted" bind:value={datePosted} class="w-full rounded border p-2">
	...
	{:else if $totalJobs > 0}
		<div class="mt-4 text-center">Loading jobs...</div>
	{/if}
</div>

...
<div class="card variant-soft-primary mb-4 p-4">
	<div
	...
	>
		...
		<button class="variant-ghost btn" aria-label={isExpanded ? 'Collapse' : 'Expand'}>
			{isExpanded ? '▼' : '▶'}
		</button>
	</div>
			...
			{#if emailRequest}
				<div class="grid grid-cols-2 gap-4">
					...
					<div class="col-span-2">
						<h4 class="mb-2 text-lg font-semibold">Basic Information</h4>
					...
					<!-- Engagement Tracking -->
					<div class="col-span-2">
						<h4 class="mb-2 text-lg font-semibold">Engagement Tracking</h4>
						<div class="grid grid-cols-2 gap-2 text-sm">
							...
							<div
								class={emailRequest.unsubscribed ? 'text-error-500' : 'text-surface-900-50-token'}
							>
							...
							<div
								class={emailRequest.spamReported ? 'text-error-500' : 'text-surface-900-50-token'}
							>
					...
					<!-- Delivery Status -->
					<div class="col-span-2">
						<h4 class="mb-2 text-lg font-semibold">Delivery Status</h4>
						<div class="grid grid-cols-2 gap-2 text-sm">
							<div><strong>Status:</strong></div>
							<div
								class={emailRequest.bounced
									? 'text-error-500'
									: emailRequest.delivered
										? 'text-success-500'
										: 'text-surface-900-50-token'}
							>
								{emailRequest.bounced
									? 'Bounced'
									: emailRequest.dropped
										? 'Dropped'
										: emailRequest.delivered
											? 'Delivered'
											: emailRequest.deferred
												? 'Deferred'
												: emailRequest.processed
													? 'Processed'
													: 'Unknown'}
							</div>
					...
					{#if emailRequest.metadata}
						<div class="col-span-2">
							<h4 class="mb-2 text-lg font-semibold">Metadata</h4>
							<div class="rounded-lg bg-surface-100 p-4">
								<pre class="whitespace-pre-wrap text-sm">{formatValue(emailRequest.metadata)}</pre>
							</div>
						</div>
					{/if}
					...
					{#if emailRequest.html}
						<div class="col-span-2">
							<h4 class="mb-2 text-lg font-semibold">HTML Content</h4>
							<div class="rounded-lg bg-surface-100 p-4">
								<pre class="whitespace-pre-wrap text-sm">{emailRequest.html}</pre>
							</div>
						</div>
					{/if}
					...
					{#if emailRequest.text}
						<div class="col-span-2">
							<h4 class="mb-2 text-lg font-semibold">Text Content</h4>
							<div class="rounded-lg bg-surface-100 p-4">
								<pre class="whitespace-pre-wrap text-sm">{emailRequest.text}</pre>
							</div>
						</div>
					{/if}
				</div>

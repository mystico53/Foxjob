		...
		<div class="card p-6">
			...
			<div class="mt-6 flex items-center justify-center gap-4">
				<div class="flex items-center gap-2">
					<div class="h-4 w-4 bg-success-500"></div>
					<span>Completed</span>
				</div>
				<div class="flex items-center gap-2">
					<div class="h-4 w-4 bg-success-500 opacity-50"></div>
					<span>Empty</span>
				</div>
				<div class="flex items-center gap-2">
					<div class="h-4 w-4 bg-primary-500 opacity-75"></div>
					<span>In Progress</span>
				</div>
				<div class="flex items-center gap-2">
					<div class="h-4 w-4 bg-warning-500 opacity-75"></div>
					<span>Timeout</span>
				</div>
				<div class="flex items-center gap-2">
					<div class="h-4 w-4 bg-error-500 opacity-75"></div>
					<span>Error</span>
				</div>
			</div>
		</div>
			...
			<div class="card mt-8 p-6">
				...
				{#if todayData && todayData.batches && todayData.batches.length > 0}
					...
					<div class="mb-6 flex flex-wrap gap-2">
						...
						>
							Empty Search ({todayData.batches.filter(
							...
							<div class="card variant-ghost p-4">
								<div class="grid grid-cols-2 gap-4 md:grid-cols-4">
									<div>
										...
										<button
											class="cursor-pointer font-mono text-sm text-primary-500 hover:underline"
									...
									<div>
										...
										<div class="flex items-center gap-2">
											<div
												class="h-3 w-3 rounded-full {batch.status === 'complete'
													? 'bg-success-500'
											...
											></div>
											<p class="capitalize">
												{batch.status === 'completed' ? 'empty' : batch.status}
											</p>
										</div>
									</div>
									<div>
										...
										<div class="flex items-center gap-2">
											<div class="h-2 w-full rounded-full bg-surface-200">
												<div
													class="h-2 rounded-full {batch.status === 'complete'
														? 'bg-success-500'
														: batch.status === 'empty' || batch.status === 'completed'
															? 'bg-success-500'
															: batch.status === 'processing'

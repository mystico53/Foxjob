...
<script>
	...
	async function checkExistingVote() {
		...
		const feedbackRef = doc(
			db,
			FEEDBACK_COLLECTION,
			`${jobId}_${sanitizedItemId}_${auth.currentUser.uid}`
		);
...
<div class="inline-flex items-center gap-1">
	...
	{/if}
	<button
		class="variant-ghost-tertiary btn-icon transition-colors {userVote === 'upvote'
			? 'text-gray-800'
			: 'hover:text-grey-800 text-gray-400'}"
	...
	<button
		class="variant-ghost-tertiary btn-icon transition-colors {userVote === 'downvote'
			? 'text-red-600'
